...
export function verifySlackSignature(rawBody: string, timestamp: string, signature: string, secret: string): boolean {
...
app.post('/command', async (c) => {
  ...
  if (!secret) {
    return c.json({ response_type: 'ephemeral', text: 'Slack is not configured on this server (set SLACK_SIGNING_SECRET).' });
  }
  ...
  if (!text) {
    return c.json({ response_type: 'ephemeral', text: 'Ask me something: `/brain how do we cut a release?`' });
  }
  ...
  const sources = res.citations
    .slice(0, 3)
    .map((cit, i) => `${i + 1}. ${cit.title ?? 'Untitled'}${cit.sourceUrl ? ` — ${cit.sourceUrl}` : ''}`)
    .join('\n');
  ...
  return c.json({
    ...
    blocks: [
      ...
      ...(sources ? [{ type: 'context', elements: [{ type: 'mrkdwn', text: `Sources:\n${sources}` }] }] : []),
    ],
  });
});
