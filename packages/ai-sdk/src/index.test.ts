...
function mockClient() {
  return {
    search: vi.fn(async () => ({
      hits: [{ document: { title: 'Release', sourceUrl: null }, content: 'ship thursdays', score: 0.9123, documentId: 'd1' }],
    })),
    chat: vi.fn(async () => ({ message: 'We ship on Thursdays.', citations: [{ title: 'Release', sourceUrl: 'https://x' }] })),
...
describe('companyBrainTools', () => {
  ...
  it('searchMemory maps hits and rounds the score to three places', async () => {
    const tools = companyBrainTools(mockClient());
    const out = (await (tools.searchMemory.execute as Exec)({ query: 'release' }, {})) as Array<Record<string, unknown>>;
    expect(out).toHaveLength(1);
    expect(out[0]).toMatchObject({ title: 'Release', content: 'ship thursdays', documentId: 'd1', source: null });
    expect(out[0]!.score).toBe(0.912);
  });

  it('askMemory returns the answer and mapped citations', async () => {
    const tools = companyBrainTools(mockClient());
    const out = (await (tools.askMemory.execute as Exec)({ question: 'when?' }, {})) as { answer: string; citations: unknown[] };
