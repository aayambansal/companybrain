  ...
  configSchema: [
    {
      ...
      required: true,
      placeholder: 'acme',
      help: 'The <domain> in https://<domain>.pipedrive.com.',
    },
    {
      key: 'apiToken',
      label: 'API token',
      type: 'password',
      required: true,
      help: 'Find under Pipedrive > Settings > Personal preferences > API.',
    },
  ],
  async *pull(ctx) {
    ...
    if (!domain || !apiToken)
      throw new Error('pipedrive connector: domain and apiToken are required');
    ...
    while (start !== undefined) {
      ...
      const res: PipedriveNotesResponse = await fetchJson<PipedriveNotesResponse>(url, {
        signal: ctx.signal,
      });
      ...
      start =
        pagination?.more_items_in_collection && pagination.next_start != null
